...
function AccordionTrigger({
  ...
  return (
    <TextClassContext.Provider
      value={cn(
        'text-left font-sans-semibold text-base text-white',
      ...
      <AccordionPrimitive.Header>
        <AccordionPrimitive.Trigger {...props} asChild>
          <Trigger
            className={cn(
              'flex-row items-center justify-between gap-4 py-4 disabled:opacity-50',
              Platform.select({
...
function AccordionContent({
  ...
  const { isExpanded } = AccordionPrimitive.useItemContext();
  return (
    <TextClassContext.Provider value="font-sans text-sm text-zinc-500 leading-6">
